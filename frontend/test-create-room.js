...
(async () => {
  ...
  // Check for max label (should show total players count, e.g., "20")
  ...
  // ==========================================
  // TEST: Click "Старт игры" button and verify room creation
  // ==========================================
  console.log('\n=== Testing room creation ===');
  
  const startButton2 = await page.locator('button:has-text("Старт игры")');
  const isDisabled = await startButton2.isDisabled();
  ...
  if (!isDisabled) {
    ...
    if (currentUrl.includes('/lobby')) {
      console.log('✓ Successfully created room and joined lobby!');
      console.log(`  Room URL: ${currentUrl}`);
    } else {
      console.log('✗ Did not navigate to lobby');
    }
  } else {
    console.log('✗ Start button is disabled, cannot create room');
  }
  
  // Take screenshot
  console.log('\n=== Taking screenshot ===');
  await page.screenshot({ path: 'test-create-room-screenshot.png', fullPage: true });
  console.log('Screenshot saved to test-create-room-screenshot.png');
